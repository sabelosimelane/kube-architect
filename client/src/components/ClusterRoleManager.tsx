...
import type { KubernetesClusterRole, PolicyRule, ClusterRoleFormData } from '../types';
import { API_GROUPS, ALL_VERBS, VERB_DESCRIPTIONS, COMMON_CLUSTER_ROLE_TEMPLATES } from '../data/kubernetesRBAC';

interface ClusterRoleManagerProps {
  clusterRoles: KubernetesClusterRole[];
  onAddClusterRole: (clusterRole: KubernetesClusterRole) => void;
  onUpdateClusterRole: (clusterRole: KubernetesClusterRole, index: number) => void;
  onDeleteClusterRole: (clusterRoleName: string) => void;
  onClose: () => void;
  editingIndex?: number;
}

export function ClusterRoleManager({ 
  clusterRoles, 
  onAddClusterRole, 
  onUpdateClusterRole, 
  onDeleteClusterRole, 
  onClose,
  editingIndex 
}: ClusterRoleManagerProps) {
  const [clusterRoleFormData, setClusterRoleFormData] = useState<ClusterRoleFormData>(() => {
    if (editingIndex !== undefined && clusterRoles[editingIndex]) {
      const clusterRole = clusterRoles[editingIndex];
      return {
        metadata: {
  ...
  const validateClusterRole = (): boolean => {
    const newErrors: Record<string, string> = {};
    
    if (!clusterRoleFormData.metadata.name.trim()) {
      newErrors.name = 'ClusterRole name is required';
    } else if (!/^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/.test(clusterRoleFormData.metadata.name)) {
      newErrors.name = 'Use only lowercase letters, numbers, and hyphens';
    } else if (editingIndex === undefined && clusterRoles.some(r => r.metadata.name === clusterRoleFormData.metadata.name)) {
      newErrors.name = 'ClusterRole already exists';
    }
    
    clusterRoleFormData.rules.forEach((rule, index) => {
  ...
  const applyTemplate = (template: typeof COMMON_CLUSTER_ROLE_TEMPLATES[0]) => {
    setClusterRoleFormData(prev => ({
      ...prev,
  ...
  const handleCreateOrUpdateClusterRole = () => {
    if (!validateClusterRole()) return;

    const clusterRole: KubernetesClusterRole = {
      apiVersion: 'rbac.authorization.k8s.io/v1',
      kind: 'ClusterRole',
      metadata: {
        name: clusterRoleFormData.metadata.name,
        ...(Object.keys(clusterRoleFormData.metadata.labels).length > 0 && {
          labels: clusterRoleFormData.metadata.labels
        })
      },
      rules: clusterRoleFormData.rules
    };

    if (editingIndex !== undefined) {
      onUpdateClusterRole(clusterRole, editingIndex);
    } else {
      onAddClusterRole(clusterRole);
    }

    // Reset form
    setClusterRoleFormData({
  ...
  const handleDeleteClusterRole = (clusterRoleName: string) => {
    onDeleteClusterRole(clusterRoleName);
  ...
  const getPermissionSummary = (rule: PolicyRule) => {
    ...
    if (rule.verbs.includes('*')) {
      return `Full cluster access to ${resources} in ${apiGroup === '' ? 'core' : apiGroup} API`;
    } else if (rule.verbs.every(v => ['get', 'list', 'watch'].includes(v))) {
      return `Read-only cluster access to ${resources} in ${apiGroup === '' ? 'core' : apiGroup} API`;
    } else {
      return `Can ${verbs} on ${resources} in ${apiGroup === '' ? 'core' : apiGroup} API (cluster-wide)`;
    }
  };
  ...
  const getWizardStepDescription = () => {
    switch (wizardStep) {
      case 0: return 'First, choose which Kubernetes API group this rule will apply to (cluster-wide).';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] min-h-[600px] overflow-hidden flex flex-col">
        {/* Header */}
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">
                {editingIndex !== undefined ? 'Edit ClusterRole' : 'ClusterRole Manager'}
              </h3>
              <p className="text-xs text-gray-500">Create and manage Kubernetes cluster-wide RBAC ClusterRoles</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {!showWizard ? (
          <div className="flex-1 overflow-y-auto">
            <div className="p-4">
              {/* Create/Edit ClusterRole Form */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-base font-semibold text-gray-900">
                    {editingIndex !== undefined ? 'Edit ClusterRole' : 'Create New ClusterRole'}
                  </h4>
                  <button
                    type="button"
                ...
                {showTemplates && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                    <h5 className="text-xs font-medium text-gray-700 mb-2">ClusterRole Templates</h5>
                    <div className="space-y-1">
                      {COMMON_CLUSTER_ROLE_TEMPLATES.map((template, index) => (
                        <button
                          key={index}
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                  <h5 className="text-xs font-medium text-gray-700">ClusterRole Details</h5>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={clusterRoleFormData.metadata.name}
                      onChange={(e) => setClusterRoleFormData(prev => ({
                        ...prev,
                        metadata: { ...prev.metadata, name: e.target.value }
                      }))}
                      className="w-full px-2 py-1.5 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      ClusterRoles are cluster-scoped and don't belong to any namespace
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="inline-flex items-center px-2 py-1 bg-orange-600 text-white rounded-md text-xs hover:bg-orange-700 disabled:opacity-50"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="px-2 py-1.5 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                      placeholder="key"
                    />
                    <input
                      type="text"
                  ...
                  {Object.entries(clusterRoleFormData.metadata.labels).length > 0 && (
                    <div className="space-y-1">
                      {Object.entries(clusterRoleFormData.metadata.labels).map(([key, value]) => (
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={startWizard}
                      className="inline-flex items-center px-2 py-1 bg-orange-600 text-white rounded-md text-xs hover:bg-orange-700"
                    >
                      <Plus className="w-3 h-3 mr-1" />
                      Add Rule
                    </button>
                  ...
                  {/* Rules List */}
                  {clusterRoleFormData.rules.length > 0 ? (
                    <div className="space-y-2">
                      {clusterRoleFormData.rules.map((rule, index) => (
                            ...
                            <div className="flex items-center space-x-2 mb-1">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {rule.resources.map((resource, resourceIndex) => (
                                  <span key={resourceIndex} className="px-1.5 py-0.5 bg-gray-100 text-gray-700 rounded text-xs">
                                    {resource}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {rule.verbs.map((verb, verbIndex) => (
                                  <span key={verbIndex} className={`px-1.5 py-0.5 rounded text-xs border ${getVerbColor(verb)}`}>
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm font-medium">No permission rules yet</p>
                      <p className="text-xs mt-1">Click "Add Rule" to create your first cluster-wide permission rule</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:ring-2 focus:ring-orange-500"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={handleCreateOrUpdateClusterRole}
                  className="px-4 py-2 text-sm font-medium text-white bg-orange-600 border border-transparent rounded-md hover:bg-orange-700 focus:ring-2 focus:ring-orange-500"
                >
                  {editingIndex !== undefined ? 'Update ClusterRole' : 'Create ClusterRole'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-lg font-semibold text-gray-900">Add Rule - ClusterRole Wizard</h4>
                <button
                  onClick={closeWizard}
                ...
                {wizardStep === 2 && wizardData.verbs.length > 0 && (
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">
                      {wizardData.verbs.includes('*') ? 'Full cluster access' : 
                       wizardData.verbs.every(v => ['get', 'list', 'watch'].includes(v)) ? 'Read-only cluster access' :
                       'Custom cluster permissions'}
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60">
            <div className="bg-white rounded-lg p-4 max-w-sm w-full mx-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Are you sure you want to delete the ClusterRole <strong>"{deleteConfirm}"</strong>?
              </p>
              <div className="flex space-x-3">
                <button
                  onClick={() => deleteConfirm && handleDeleteClusterRole(deleteConfirm)}

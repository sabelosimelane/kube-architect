...
export function RolesList({
  ...
  const filteredRoles = namespaceFilter
    ? roles.filter(role => role.metadata.namespace === namespaceFilter)
    : roles;

  const getOriginalIndex = (role: KubernetesRole) => {
  ...
  if (filteredRoles.length === 0 && namespaceFilter) {
    return (
      <div className="p-8 text-center text-gray-500 dark:text-gray-400">
        <p>No Roles found in namespace "{namespaceFilter}"</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2 p-4">
      {filteredRoles.map((role) => {
        const index = getOriginalIndex(role);
        return (
          <button
            key={index}
            onClick={() => onSelect(index)}
            className={`p-3 border rounded-lg cursor-pointer transition-all w-full text-left duration-200 hover:shadow-md ${selectedIndex === index

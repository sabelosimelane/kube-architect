...
export function SecretsList({
  ...
  const filteredSecrets = namespaceFilter
    ? secrets.filter(s => s.namespace === namespaceFilter)
    : secrets;

  const getOriginalIndex = (secret: Secret) => {
  ...
  if (filteredSecrets.length === 0 && namespaceFilter) {
    return (
      <div className="p-8 text-center text-gray-500 dark:text-gray-400">
        <p>No Secrets found in namespace "{namespaceFilter}"</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1 p-4">
      {filteredSecrets.map((secret) => {
        const index = getOriginalIndex(secret);
        return (
          <button
            key={secret.name}

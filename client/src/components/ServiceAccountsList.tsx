...
export function ServiceAccountsList({
  ...
  const filteredServiceAccounts = namespaceFilter
    ? serviceAccounts.filter(sa => sa.namespace === namespaceFilter)
    : serviceAccounts;

  const getOriginalIndex = (serviceAccount: ServiceAccount) => {
    return serviceAccounts.indexOf(serviceAccount);
  };
  ...
  if (filteredServiceAccounts.length === 0 && namespaceFilter) {
    return (
      <div className="p-8 text-center text-gray-500 dark:text-gray-400">
        <p>No Service Accounts found in namespace "{namespaceFilter}"</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 p-4">
      {filteredServiceAccounts.map((serviceAccount) => {
        const index = getOriginalIndex(serviceAccount);
        return (
          <button
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3 min-w-0 flex-1">
                <div className="flex-shrink-0">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${selectedIndex === index ? 'bg-cyan-600' : 'bg-gray-100 dark:bg-gray-700'
                    }`}>
                    <K8sServiceAccountIcon className={`w-4 h-4 ${selectedIndex === index ? 'text-white' : 'text-gray-600 dark:text-gray-300'
                      }`} />
                  </div>
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-sm text-gray-500 flex items-center space-x-2 mt-1 dark:text-gray-200">
                    ...
                    <span>•</span>
                    <span className={`px-1 py-0.5 rounded text-xs font-medium ${serviceAccount.automountServiceAccountToken !== false
